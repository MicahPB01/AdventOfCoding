...
public class PartTwo {
    ...
    private static char findMostFrequentCard(String currentCards)   {

        System.out.println("CURRENTS: " + currentCards);
        ...
        for(int j = 0; j < currentCards.length(); j++) {
            ...
            char currentCard = currentCards.charAt(j);
            //System.out.println("Looking at: " + currentCard);
            if(currentCard == 'J')   {
                numJ++;
                continue;
            }
            ...
            for (int i = 0; i < currentCards.length(); i++) {
                ...
                if (currentCards.charAt(i) == currentCard)   {
                    count++;
                }
            ...
            System.out.println("Card this: " + currentCard + " has " + count);

            if(count > maxCount)   {
                System.out.println("Updating frequent card");
                maxCount = count;
        ...
        if(numJ >= 1 && maxCount == 1)   {
            mostFrequent = findHighestCard(currentCards);
        }
    ...
    private static char findHighestCard(String currentCards) {
        String cardOrder = "J23456789TQKA";
        int highestCardIndex = 0; // Index of 'J' in cardOrder

        for (int i = 0; i < currentCards.length(); i++) {
            char currentCard = currentCards.charAt(i);
            int currentIndex = cardOrder.indexOf(currentCard);

            if (currentIndex > highestCardIndex) {
                highestCardIndex = currentIndex;
            }
        }

        return cardOrder.charAt(highestCardIndex);
    }
    ...
    private static String startNewHand(String currentCards)   {
        ...
        for(int i = 0; i < currentCards.length(); i++)   {
            ...
            if(currentCards.charAt(i) == 'J')   {
                currentCard = findMostFrequentCard(currentCards);
                numberOfJ++;
            }
            newHand += currentCard;
        ...
        System.out.println("NEW HAND IS: " + newHand);
    ...
    private static Hand determineHand(Hand hand)   {
        ...
        int loopControl = 0;
        String keepCards = hand.getCards();

        String currentCards = hand.getCards();

        currentCards = startNewHand(currentCards);

        hand.setCards(currentCards);
